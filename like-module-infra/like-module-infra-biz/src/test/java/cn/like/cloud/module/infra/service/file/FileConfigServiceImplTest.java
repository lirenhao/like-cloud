package cn.like.cloud.module.infra.service.file;
...
import cn.like.cloud.framework.file.core.client.FileClient;
import cn.like.cloud.framework.file.core.client.FileClientConfig;
import cn.like.cloud.framework.file.core.client.FileClientFactory;
import cn.like.cloud.framework.file.core.client.local.LocalFileClient;
import cn.like.cloud.framework.file.core.client.local.LocalFileClientConfig;
import cn.like.cloud.framework.file.core.enums.FileStorageEnum;
import cn.like.cloud.framework.test.core.ut.BaseDbUnitTest;
import cn.like.cloud.module.infra.controller.admin.file.vo.config.FileConfigCreateReqVO;
import cn.like.cloud.module.infra.controller.admin.file.vo.config.FileConfigPageReqVO;
...
import cn.like.cloud.module.infra.dal.mysql.file.FileConfigMapper;
import lombok.Data;
...
import static cn.like.cloud.framework.common.util.date.LocalDateTimeUtils.buildTime;
import static cn.like.cloud.framework.common.util.object.ObjectUtils.cloneIgnoreId;
import static cn.like.cloud.framework.test.core.util.AssertUtils.assertPojoEquals;
import static cn.like.cloud.framework.test.core.util.AssertUtils.assertServiceException;
import static cn.like.cloud.framework.test.core.util.RandomUtils.randomLongId;
import static cn.like.cloud.framework.test.core.util.RandomUtils.randomPojo;
import static cn.like.cloud.module.infra.enums.ErrorCodeConstants.FILE_CONFIG_DELETE_FAIL_MASTER;
import static cn.like.cloud.module.infra.enums.ErrorCodeConstants.FILE_CONFIG_NOT_EXISTS;
...
public class FileConfigServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateFileConfig_success() {
        // 准备参数
        Map<String, Object> config = MapUtil.<String, Object>builder().put("basePath", "/like")
                .put("domain", "https://www.bing.cn").build();
        ...
        assertEquals("/like", ((LocalFileClientConfig) fileConfig.getConfig()).getBasePath());
        assertEquals("https://www.bing.cn", ((LocalFileClientConfig) fileConfig.getConfig()).getDomain());
        // 验证 cache
    ...
    public void testUpdateFileConfig_success() {
        ...
        FileConfigDO dbFileConfig = randomPojo(FileConfigDO.class, o -> o.setStorage(FileStorageEnum.LOCAL.getStorage())
                .setConfig(new LocalFileClientConfig().setBasePath("/like").setDomain("https://www.bing.cn")));
        fileConfigMapper.insert(dbFileConfig);// @Sql: 先插入出一条存在的数据
        // 准备参数
        FileConfigUpdateReqVO reqVO = randomPojo(FileConfigUpdateReqVO.class, o -> {
            o.setId(dbFileConfig.getId()); // 设置更新的 ID
            Map<String, Object> config = MapUtil.<String, Object>builder().put("basePath", "/like2")
                    .put("domain", "https://doc.bing.cn").build();
            o.setConfig(config);
        });
        ...
        assertPojoEquals(reqVO, fileConfig, "config");
        assertEquals("/like2", ((LocalFileClientConfig) fileConfig.getConfig()).getBasePath());
        assertEquals("https://doc.bing.cn", ((LocalFileClientConfig) fileConfig.getConfig()).getDomain());
        // 验证 cache
    ...
    public void testGetFileConfigPage() {
        // mock 数据
        FileConfigDO dbFileConfig = randomFileConfigDO().setName("测试数据")
        ...
        // 测试 name 不匹配
        fileConfigMapper.insert(cloneIgnoreId(dbFileConfig, o -> o.setName("数据")));
        // 测试 storage 不匹配
    ...
    public void testFileConfig() throws Exception {
        ...
        when(fileClient.upload(any(), any(), any())).thenReturn("https://www.bing.cn");

        // 调用，并断言
        assertEquals("https://www.bing.cn", fileConfigService.testFileConfig(id));
    }
    ...
    private FileConfigDO randomFileConfigDO() {
        return randomPojo(FileConfigDO.class).setStorage(RandomUtil.randomEle(FileStorageEnum.values()).getStorage())
